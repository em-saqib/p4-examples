...
control MyIngress(inout headers hdr,
    ...
    // 1. Declare reigsters
    register<bit<16>>(1000) flows_vector;
...
action be_forward_a(macAddr_t dstAddr, egressSpec_t port) {
    standard_metadata.egress_spec = port;
    hdr.ethernet.srcAddr = hdr.ethernet.dstAddr;
    hdr.ethernet.dstAddr = dstAddr;
    }
...
apply {
    if (hdr.ipv4.isValid() ) {
            ...
            if(hdr.ipv4.protocol ==6) {
                ...
                log_msg(" Flow Record srcIP : {} dstIP : {} srcPort : {} dstPort : {} ", {hdr.ipv4.srcAddr, hdr.ipv4.dstAddr, hdr.tcp.srcPort, hdr.tcp.dstPort});
