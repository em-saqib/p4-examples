...
apply {
          ...
          hash(metadata.flowID,
              ...
              metadata.packetSize = hdr.ipv4.len - 51;  // 40 is IP and 12 is INT size

              log_msg(" INFO FlowID : {} packetCounter : {} Size : {} ", {metadata.flowID, metadata.packet_counter, hdr.ipv4.len - 51});
              ...
              if (metadata.packet_counter > 4 && metadata.classID != 0){
                  ...
                  packet_counter_reg.write((bit<32>)metadata.flowID, metadata.packet_counter);
                      ...
                      if (metadata.packet_counter == 4){
                          ...
                          //hdr.int_header.latency = (bit<32>)metadata.classID * 100;

                          //log_msg(" INFO ClassID : {} Latency : {} ", {hdr.int_header.class, latency});
