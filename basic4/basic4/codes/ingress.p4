        ...
        //feature1_exact.apply();
            if(hdr.ipv4.protocol ==6) {
                ...
                hash(meta.flowID,
                    ...
                    // FlowID check codition is to not process the garbage flows, currently it is flow 524
                    if (meta.flowID != 524){

                    // 3. Read packet size and keep in metadata
                    //meta.packetSize = hdr.ipv4.totalLen - 40;
                    //log_msg(" INFO FlowID : {} PackSize : {}", {meta.flowID, hdr.ipv4.totalLen});
                    ...
                    //log_msg(" INFO FlowID : {} PackCounter : {} PackSize : {}", {meta.flowID, meta.packet_counter, hdr.ipv4.totalLen});
                    ...
                    if (meta.packet_counter > 4){
                        ...
                        // This counter is for troublshooting purpose to counts packets for each flow
                        packet_counter_reg.write((bit<32>)meta.flowID, meta.packet_counter);
                        ...
                        if (meta.classID != 0){
                            ...
                            log_msg(" INFO F_ID : {} C_ID : {} F_Port : {} P_Counter : {}", {meta.flowID, meta.classID, hdr.tcp.clasID, meta.packet_counter});
                        }
                    }
