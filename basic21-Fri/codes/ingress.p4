...
control MyIngress(inout headers hdr,
    ...
    apply {
        if (hdr.ipv4.isValid()) {
            ...
            //standard_metadata.priority = (bit<8>)hdr.int_header.class;
            ...
            log_msg(" INFO FlowID :{} ClassID : {} PacketCounter : {} Latency : {} qDelay : {} ", {metadata.flowID, hdr.int_header.class, metadata.packet_counter, hdr.int_header.latency, metadata.current_queue_delay});
...
control MyEgress(inout headers hdr,
    ...
    apply {
          ...
          procTime_reg.write(0, metadata.procTime);

          // Update Latency by substracting queue delay
          if (hdr.int_header.latency > standard_metadata.deq_timedelta){
            hdr.int_header.latency = hdr.int_header.latency - (bit<32>)standard_metadata.deq_timedelta;   // Process time included queueing delay
          }

          bit<3> qid = standard_metadata.priority;
          queue_delay.write((bit<32>)qid, standard_metadata.deq_timedelta);

          log_msg(" INFO QUEUE: Q_ID : {} EnqDepth:{} DeqDepth : {} Q_Delay : {} ", {standard_metadata.priority, standard_metadata.enq_qdepth, standard_metadata.deq_qdepth, standard_metadata.deq_timedelta});
